...
{
    class Wrapper_localhost_PLM_JPC_WF_AssignmentMetrix
    {
        /// <summary>
        /// 初始化
        /// </summary>
        ...
        public Case_JPC_WF_AssignmentMetrix init(HttpServerConnection connection)
        {
            Case_JPC_WF_AssignmentMetrix innMethod = new Case_JPC_WF_AssignmentMetrix(connection);
            return innMethod;
        }
        public class Case_JPC_WF_AssignmentMetrix : Item
        {
            public Aras.Server.Core.CallContext CCO { get; set; }
            public Aras.Server.Core.IContextState RequestState { get; set; }
            public Case_JPC_WF_AssignmentMetrix(IServerConnection arg) : base(arg)
            {
            }
            ...
            public Item MethodCode0()
            {
                ...
                private Item GetAssignmentSetting(string assign_act_id){
                    string aml = @"<AML>
                                <Item action='get' type='Assignment' orderBy='sort_order'>
                ...
                private Item GetAddIdentityByItemField(Item formItm,string prop1,string prop2){
                    string prop1_id = formItm.getProperty(prop1,"");
                    string prop1_type = formItm.getPropertyAttribute(prop1,"type","");
                    string owned_by_id_dept = formItm.getProperty("owned_by_id_dept", "");
                    Item prop1_item = inn.newItem(prop1_type,"get");
                            prop1_item.setProperty("id",prop1_id);
                            prop1_item = prop1_item.apply();
                    if(prop1_item.isError()){
                        ...
                        //假如 owned_by_id_dept 有值，也許有跨部門
                        if (owned_by_id_dept != "")
                        {
                            if(owned_by_id_dept != prop1_item.getProperty("cn_main_dept", ""))
                            {
                                Item crossAssignment = GetCrossAssignment(prop1_id, owned_by_id_dept);
                                if (crossAssignment.isError() == false)
                                {
                                    prop1_item = crossAssignment;
                                }
                            }
                        }
                ...
                public Item GetCrossAssignment(string owned_by_id, string owned_dept)
                {
                    string aml = @"
                    <AML>
                        <Item type='JPC_CrossAssignment' action='get'>
                            <cn_main_dept>{0}</cn_main_dept>
                            <Relationships>
                                <Item type='JPC_CrossAssignment_Identity' action='get'>
                    ...
                    ";
                    aml = string.Format(aml, owned_dept, owned_by_id);
                    Item itmCrossAssignment = inn.applyAML(aml);
                    return itmCrossAssignment;
                }
                private Item GetAddIdentityByRelationshipField(Item formItm,string relationship_name,string field_name){
                    Item itmRelationNoRelated = inn.newItem(relationship_name,"get");
                    itmRelationNoRelated.setProperty("source_id",formItm.getID());
                    itmRelationNoRelated.setAttribute("select",field_name+"(id,keyed_name)");
                    itmRelationNoRelated = itmRelationNoRelated.apply();
                    if(itmRelationNoRelated.isError()) return null;

                    return itmRelationNoRelated.getItemsByXPath("//Item/"+field_name+"/Item");
                }
                private bool CheckPropValue(Item formItm,string field_name,string condition,string field_value){
                    ...
                    switch(condition){
                        ...
                        break;
                        case "<>":
                            //Log(num.ToString()+"<>"+field_value_D.ToString());
                            if(num != field_value_D) return true;
                        break;
                        case ">=":
                            if(num >= field_value_D) return true;
                ...
                private Item main(Item thisItem){
                    ...
                    if(AssignmentActSetting.isError()==false){
                        ...
                        //return inn.newError(AssignSettingItms.ToString());
                        for(int i=0;i<AssignSettingItms.getItemCount();i++){
                            ...
                            string jpc_no_support = assign_set.getProperty("jpc_no_support","0");
                            if(jpc_no_support=="1"){
                                continue;
                            }
                            //return inn.newError(jpc_method);
                            Item newIdentity = inn.newItem();
                            string body = @"<form_id>{0}</form_id>
                                                            <form_type>{1}</form_type>
                                                            <p1>{2}</p1>
                                                            <p2>{3}</p2>
                                                            <owned_by_id_dept>{4}</owned_by_id_dept>";
                            ...
                            switch(assignment_type){
                                ...
                                case "Item Field":
                                    ...
                                    if(newIdentity==null){
                                        body = string.Format(body,formItm.getID()
                                                                , formItm.getAttribute("type","")
                                                                , parameter1
                                                                , parameter2
                                                                , formItm.getProperty("owned_by_id_dept", "")
                                                            );
                                        if(jpc_method!="")
                                        {
                                ...
                                break;
                                case "Relationship_No_Related":
                                    //變數1=Relationship Name,變數2=Relationship 的 Item欄位
                                    Item newIdentitys = GetAddIdentityByRelationshipField(formItm,parameter1,parameter2);
                                    //return inn.newError(newIdentitys.ToString());
                                    if(newIdentitys!=null){
                                        if(newIdentitys.isError()==false){
                                            for(int index=0;index < newIdentitys.getItemCount();index++){
                                                newIdentity = newIdentitys.getItemByIndex(index);
                                                if(CheckAddedAssignment(item_id,newIdentity.getID())==false)
                                                {
                                                    Item newActAssignment = inn.newItem("Activity Assignment","add");
                                                    newActAssignment.setProperty("source_id",item_id);
                                        ...
                                        try{
                                            if(newIdentity.isError()==false && newIdentity.getResult().ToString().Trim()!=""){
                                                if(CheckAddedAssignment(item_id,newIdentity.getID())==false)
                                                {
                                                    ...
                                                    newActAssignment.setProperty("voting_weight",voting_weight);
                                                    newActAssignment = newActAssignment.apply();
                                                    if(newIdentity.getID()=="0F7420276B844FA199F94F8428640470") //wilson 張偉三
                                                    {
                                                        //AddAssignment(thisItem.getID(),"F7DE1E0BA6C9462BBB4925893A4C33E6",is_required,for_all_members,voting_weight);
                                                    }
                                                    //return inn.newError(check.ToString());
                                                }
                                            }
                                    ...
                                    if(newIdentity.isError()==false && newIdentity.ToString().Trim()!=""){
                                        if(CheckAddedAssignment(item_id,newIdentity.getID())==false)
                                        {
                                            if(newIdentity.isError()==false){
                                                ...
                                                newActAssignment.setProperty("voting_weight",voting_weight);
                                                newActAssignment = newActAssignment.apply();
                                                if(newIdentity.getProperty("id","")=="0F7420276B844FA199F94F8428640470") //wilson 張偉三
                                                {
                                                    AddAssignment(item_id,"F7DE1E0BA6C9462BBB4925893A4C33E6",is_required,for_all_members,voting_weight);
                                                }
                                            }
                                        }
                                    }
                        ...
                        if(ChangePathItems.isError()==false){
                            ...
                            for(int i=0;i<ChangePathItems.getItemCount();i++){
                                ...
                                if(check==true)
                                {
                                    ...
                                    if(check_more){
                                        Item nextPaths = GetNextPaths(item_id);
